...
const SHARE_LINKS = { "WsZXNAC9aJw6tSKUqsLa": "disneyland", "jhzOWz6P9kKtD5MH95JV": "prestige", "4EOkMMU00Xrus1qzLhaQ": "ile-maurice", "4gvW36Rs9nOGTRUrHR9A": "dubai" };
...
const [affPct, setAffPct] = useState("");
const [affType, setAffType] = useState("percent");const [showSocial, setShowSocial] = useState(false);
...
const akey = (Math.random().toString(36).slice(2)+Math.random().toString(36).slice(2)).slice(0,10).toUpperCase();
  await setDoc(doc(db,"affiliates",c), { code:c, name:affName.trim(), commissionType:affType, commissionValue:p, accessKey:akey, paidCommission:0, active:true, createdAt: serverTimestamp() }, { merge:true });  var newAff = { code:c, name:affName.trim(), commissionType:affType, commissionValue:p, accessKey:akey };
...
<div style={{fontSize:"11px",color:C.textLt,marginTop:"3px"}}>Clé d'acces : <span style={{fontFamily:"monospace",color:C.textMd}}>{aff.accessKey||"—"}</span></div>
              <button onClick={function(e){ var btn=e.currentTarget; var comm=(aff.commissionType==="perticket")?((aff.commissionValue||0)+"£ par ticket vendu"):((aff.commissionValue||0)+"% du chiffre d'affaires"); var msg="Bonjour 👋\n\nBienvenue dans le programme partenaire Olawin ! Voici tes accès :\n\n🔗 TON LIEN À PARTAGER\nhttps://www.olawin.org/?ref="+(aff.code||"")+"\nPartage ce lien sur tes réseaux (Instagram, WhatsApp, TikTok…). Toutes les ventes faites via ce lien te sont automatiquement attribuées.\n\n📊 TON ESPACE INFLUENCEUR (pour suivre tes ventes et ta commission)\nhttps://www.olawin.org/?pro\nCode : "+(aff.code||"")+"\nClé d'accès : "+(aff.accessKey||"")+"\n\n💰 Ta commission : "+comm+".\n\nÀ très vite,\nL'équipe Olawin"; navigator.clipboard.writeText(msg).then(function(){ var old=btn.innerText; btn.innerText="✓ Message copié !"; setTimeout(function(){ btn.innerText=old; },1800); }); }} style={{marginTop:"8px",background:C.btnBg,color:C.btnText,border:"none",borderRadius:"7px",padding:"7px 12px",fontSize:"11px",fontWeight:"700",cursor:"pointer"}}>📋 Copier le message influenceur</button>
<div style={{marginTop:"8px"}}>
{aff.contractUrl ? (
<div style={{fontSize:"11px",color:C.textMd,display:"flex",alignItems:"center",gap:"8px",flexWrap:"wrap"}}>
<span>✅ Contrat signé{aff.contractSignedAt?(" le "+fmtD(aff.contractSignedAt)):""}</span>
<a href={aff.contractUrl} target="_blank" rel="noopener noreferrer" style={{color:C.text,textDecoration:"underline"}}>📄 Voir</a>
<button onClick={function(){ var u=window.prompt("Nouveau lien du contrat signé :", aff.contractUrl||""); if(u!==null){ updateDoc(doc(db,"affiliates",aff.id),{contractUrl:u.trim(), contractSignedAt:new Date().toISOString()}).then(function(){ notify("Contrat mis a jour ✓"); }); } }} style={{background:"none",border:"none",color:C.textLt,cursor:"pointer",fontSize:"11px",textDecoration:"underline",padding:0}}>modifier</button>
</div>
) : (
<button onClick={function(){ var u=window.prompt("Colle le lien du contrat signé (depuis SignWell) :"); if(u&&u.trim()){ updateDoc(doc(db,"affiliates",aff.id),{contractUrl:u.trim(), contractSignedAt:new Date().toISOString()}).then(function(){ notify("Contrat signé enregistré ✓"); }); } }} style={{background:"none",border:`1px dashed ${C.border}`,color:C.textMd,cursor:"pointer",fontSize:"11px",borderRadius:"7px",padding:"6px 10px"}}>➕ Marquer le contrat signé</button>
)}
</div></div>
<div style={{display:"flex",gap:"8px"}}>
...
<button onClick={()=>{const next=!d.soldOut;if(!window.confirm(next?"Afficher SOLD OUT sur ce tirage ?":"Retirer le SOLD OUT de ce tirage ?"))return;updateDoc(doc(db,"draws",d.id),{soldOut:next}).then(()=>notify(next?"SOLD OUT active":"SOLD OUT retire"));}} style={{...btn,background:d.soldOut?"#C0392B":"rgba(0,0,0,0.06)",color:d.soldOut?"#fff":C.textMd,border:d.soldOut?"none":`1px solid ${C.border}`,padding:"11px 20px",fontSize:"12px",cursor:"pointer",marginLeft:"8px"}}>{d.soldOut?"❌ RETIRER SOLD OUT":"🔴 AFFICHER SOLD OUT"}</button><span style={{fontSize:"11px",color:C.textLt}}>Cloture: {fmtD(d.endDate)} · Tirage: {fmtD(d.drawDate)}</span>
