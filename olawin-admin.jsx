...
<button onClick={async()=>{
  if(!window.confirm("Lancer le tirage automatique pour ce tirage ?\n\nLe gagnant sera tire au sort et enregistre.")) return;
  if(!window.confirm("Es-tu VRAIMENT sur ? Cette action est definitive et designera le gagnant officiel.")) return;
  notify("Tirage en cours...");
  try {
    const r = await fetch("/api/create-random-draw",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({drawId:d.id,confirmed:true})});
    const data = await r.json();
    if(data.ok && data.steps && data.steps.savedToFirebase){
      notify("🏆 Gagnant : "+data.steps.savedToFirebase.name);
    } else {
      alert("Reponse : "+JSON.stringify(data));
    }
  } catch(e){ alert("Erreur : "+e.message); }
}} style={{...btn,background:"#25ab29",color:"#fff",padding:"11px 20px",fontSize:"12px",border:"none",cursor:"pointer",marginLeft:"8px"}}>🎲 TIRAGE AUTOMATIQUE (API)</button><button onClick={()=>clearDrawOrders(d.id)} style={{...btn,background:"rgba(160,0,0,0.08)",color:"rgba(140,0,0,0.8)",border:"1px solid rgba(160,0,0,0.15)",padding:"11px 20px",fontSize:"12px",cursor:"pointer",marginLeft:"8px"}}>VIDER LES COMMANDES (TEST)</button>
</div>
{d.winner && (
