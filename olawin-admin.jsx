...
"tel aviv": "https://images.unsplash.com/photo-1598440947619-2c35fc9aa908?w=1200&q=80",
"monaco": "https://images.unsplash.com/photo-1565967511849-76a60a516170?w=1200&q=80",
...
<div style={{marginBottom:"16px"}}>
<button onClick={()=>{const NL=String.fromCharCode(10);const dOrders=orders.filter(o=>o.drawId===d.id);let csv="Numero ticket,Prenom,Nom,Email"+NL;dOrders.forEach(o=>{(o.ticketNums||[]).forEach(n=>{csv+=n+',"'+(o.firstName||"")+'","'+(o.lastName||"")+'","'+(o.email||"")+'"'+NL;});});const blob=new Blob([csv],{type:"text/csv;charset=utf-8;"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="participants.csv";document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);notify("CSV telecharge");}} style={{...btn,display:"inline-block",background:"#1A1A1A",color:"#fff",padding:"11px 20px",fontSize:"12px",marginRight:"8px",border:"none",cursor:"pointer"}}>📥 EXPORTER LES PARTICIPANTS (CSV)</button>
