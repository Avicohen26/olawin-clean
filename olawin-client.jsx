...
function DrawCard(props) {
  ...
  const soldOut = draw.soldOut || (draw.soldTickets >= draw.totalTickets);
  return (
...
<div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(0,0,0,0.92) 0%,rgba(0,0,0,0.5) 50%,rgba(0,0,0,0.15) 100%)",zIndex:2}}></div>
      {!isFinished ? (
        (soldOut ? <div style={{position:"absolute",top:"42%",left:"50%",transform:"translate(-50%,-50%)",zIndex:5,background:"#C0392B",border:"2px solid rgba(255,255,255,0.95)",borderRadius:"12px",padding:"12px 28px",fontSize:"22px",fontWeight:"900",color:"#ffffff",fontFamily:"Bebas Neue, sans-serif",letterSpacing:"3px",whiteSpace:"nowrap",boxShadow:"0 10px 30px rgba(0,0,0,0.5)"}}>SOLD OUT</div> : <div style={{position:"absolute",top:"42%",left:"50%",transform:"translate(-50%,-50%)",zIndex:5,display:"flex",alignItems:"center",gap:"7px",background:"rgba(255,255,255,0.97)",borderRadius:"24px",padding:"10px 18px",fontSize:"13px",fontWeight:"800",color:"#1A1A1A",letterSpacing:"0.2px",whiteSpace:"nowrap",boxShadow:"0 8px 22px rgba(0,0,0,0.35)",opacity:hovered?1:0.95,transition:"all 0.3s"}}>{t.cta.buyHint}<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#1A1A1A" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"></path><path d="m12 5 7 7-7 7"></path></svg></div>)
      ) : null}      <div style={{position:"absolute",inset:0,padding:"24px",display:"flex",flexDirection:"column",justifyContent:"space-between",zIndex:3}}>
...
{!isFinished ? ((draw.soldOut || (draw.soldTickets >= draw.totalTickets)) ? <div style={{background:"#C0392B",border:"2px solid rgba(255,255,255,0.95)",borderRadius:"20px",padding:"7px 18px",fontSize:"18px",color:"#ffffff",fontWeight:"800",fontFamily:"Bebas Neue, sans-serif",letterSpacing:"2px",boxShadow:"0 4px 14px rgba(192,57,43,0.55)"}}>SOLD OUT</div> : <div style={{background:"#ffffff",border:"1px solid rgba(255,255,255,0.95)",borderRadius:"20px",padding:"5px 14px",fontSize:"15px",color:"#1A1A1A",fontWeight:"800",fontFamily:"Bebas Neue, sans-serif",letterSpacing:"1.5px"}}>{draw.ticketPrice}£ / TICKET</div>) : null}        </div>
        <div>
...
<button className="nav-link" onClick={function(){ goTo("faq"); }}>{t.nav.faq}</button>          <button className="nav-link" onClick={function(){ goTo("legal"); }}>{t.nav.legal}</button>
...
<button className="nav-link" onClick={function(){ goTo("legal"); }} style={{textAlign:"left",fontSize:"14px",padding:"8px 0"}}>{t.nav.legal}</button>          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"10px",padding:"12px 0",borderTop:"1px solid rgba(0,0,0,0.08)",borderBottom:"1px solid rgba(0,0,0,0.08)"}}>
  ...
  if (page === "home") {
    if (loading) {
      ...
      pageContent = (
        <div>
          <section style={{position:"relative",minHeight:isMobile?"calc(100dvh - 64px)":"calc(100vh - 64px)",overflow:"hidden",display:"flex",flexDirection:"column",justifyContent:"flex-end",paddingTop:isMobile?"16px":"28px"}}>
            <div style={{position:"absolute",inset:0,background:(heroData && heroData.gradient) || "#1A1A1A"}}></div>
            {heroData && heroData.video ? <video autoPlay muted loop playsInline preload="auto" poster={heroData.image?optimg(heroData.image, isMobile?900:1600):undefined} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:heroData.heroPosition||"center"}}><source src={heroData.video}></source></video> : (heroData && heroData.image ? <img src={optimg(heroData.image, isMobile?900:1600)} alt={heroData.location||"Olawin"} decoding="async" onError={function(e){ imgFallback(e, heroData.image); }} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:heroData.heroPosition||"center"}}></img> : null)}
            ...
            <div style={{position:"relative",padding:isMobile?"0 20px 40px":"0 64px 72px",maxWidth:"900px"}}>
              ...
              <div style={{display:"flex",alignItems:isMobile?"stretch":"center",gap:"16px",flexWrap:"wrap",flexDirection:isMobile?"column":"row"}}>
                {featured ? ((featured.soldOut || (featured.soldTickets >= featured.totalTickets)) ? <div style={{background:"#C0392B",color:"#ffffff",padding:"16px 40px",fontSize:"20px",fontWeight:"900",fontFamily:"Bebas Neue, sans-serif",letterSpacing:"3px",borderRadius:"6px",width:isMobile?"100%":"auto",textAlign:"center",border:"2px solid rgba(255,255,255,0.9)",boxShadow:"0 10px 30px rgba(192,57,43,0.5)"}}>{lang==="fr"?"COMPLET — TIRAGE FERMÉ":lang==="es"?"AGOTADO":"SOLD OUT"}</div> : <button onClick={function(){ setSelectedDraw(featured); goTo("shop"); }} className="cta-dark" style={{background:"#FFFFFF",color:"#1A1A1A",padding:"16px 32px",fontSize:"14px",width:isMobile?"100%":"auto",fontWeight:"800"}}>{t.hero.buyTicket} {featured.ticketPrice}£</button>) : null}
      ...
      pageContent = (
        <div>
          <div style={{position:"relative",height:isMobile?"200px":"300px",overflow:"hidden",background:activeDraw.gradient||"#1A1A1A"}}>
            {activeDraw.video ? <video autoPlay muted loop playsInline preload="auto" poster={activeDraw.image?optimg(activeDraw.image, isMobile?900:1200):undefined} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}}><source src={activeDraw.video}></source></video> : (activeDraw.image ? <img src={optimg(activeDraw.image, isMobile?900:1200)} alt={activeDraw.location} decoding="async" onError={function(e){ imgFallback(e, activeDraw.image); }} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}}></img> : null)}
          ...
          <div style={{maxWidth:"1100px",margin:"0 auto",padding:isMobile?"32px 20px 60px":"40px 32px 60px"}}>
            <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"minmax(0,1fr) minmax(0,420px)",gap:isMobile?"32px":"64px",maxWidth:"100%"}}>
              <div>
                ...
                <div style={{fontSize:"15px",color:"rgba(0,0,0,0.65)",lineHeight:"1.7",marginBottom:"24px"}}>{(trd(activeDraw,"description")||"").split("\n").map(function(ln,ix){ var tx=ln.trim(); if(!tx) return <div key={ix} style={{height:"8px"}}></div>; if(tx.charAt(0)==="•") return <div key={ix} style={{display:"flex",gap:"10px",alignItems:"flex-start",marginBottom:"6px"}}><span style={{color:"#B08D57",flexShrink:0,fontWeight:700}}>•</span><span style={{flex:1}}>{tx.slice(1).trim()}</span></div>; return <p key={ix} style={{margin:"0 0 10px"}}>{tx}</p>; })}</div>
...
<div style={{fontSize:"24px",fontFamily:"Bebas Neue, sans-serif",letterSpacing:"2px"}}>{packTotal}£ <span style={{fontSize:"11px",color:isActive?"rgba(255,255,255,0.5)":"rgba(0,0,0,0.4)"}}>-{pack.discount}%</span></div>
                              {activeDraw.totalTickets ? <div style={{fontSize:"11px",letterSpacing:"1px",marginTop:"5px",color:isActive?"rgba(255,255,255,0.75)":"rgba(0,0,0,0.55)",fontFamily:"'DM Sans',sans-serif",fontWeight:"600"}}>{"1/"+Math.round(activeDraw.totalTickets/pack.qty)+(lang==="en"?" chance":lang==="es"?" de posibilidad":" de chance")}</div> : null}                             </button>                          );
                        })}
                      </div>
                      ...
                      {(activeDraw && (activeDraw.soldOut || (activeDraw.soldTickets >= activeDraw.totalTickets))) ? <div style={{width:"100%",padding:"18px",textAlign:"center",background:"#C0392B",color:"#ffffff",borderRadius:"10px",fontFamily:"Bebas Neue, sans-serif",fontSize:"20px",fontWeight:"900",letterSpacing:"3px"}}>{lang==="fr"?"COMPLET — TIRAGE FERMÉ":lang==="es"?"AGOTADO":"SOLD OUT"}</div> : <button onClick={function(){ goTo("confirm"); }} className="cta-dark" style={{width:"100%",padding:"16px",fontSize:"12px"}}>{t.shop.continueBtn} {total}£</button>}
